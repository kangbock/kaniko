	...
	connection.query(main_board, function (err, rows) {
	        if (!err) {
				...
				res.end(html)
	                ...
	                res.statusCode=302
	                res.setHeader("Location","http://www.k-tech.cloud/404.html");
	                res.end();
	            }
	})
    };	return(rows);
...
app.get('/login.html.js',function(req,res){
    if(req.session.logined){
        res.render('http://www.k-tech.cloud/logout.html',{id: req.session.user_id});
    }else 
    res.render('http://www.k-tech.cloud/login.html')
});
    // register view
app.get('/register.html',function(req,res){
    res.render('http://www.k-tech.cloud/register.html')
});
...
app.get('/logout.html.js',function(req,res){
    req.session.destroy();
    res.redirect('http://www.k-tech.cloud');
});
    ...
    connection.query('SELECT * from member where id="'+data.id+'";',function(err,rows,fields){
        ...
        if (rows.length<=0){
            ...
            connection.query('insert into member values(?,?,?,?,?,?)',params,function(err,results){
                if(err){
                    ...
                    throw err;
                     ...
                     res.send("<script>alert('success'); location.href='http://www.k-tech.cloud/login.html';</script> ");
                }
            });
        }else{
            // 아이디가 중복된다
            // 회원가입 실패. 에러를 띄우고 회원가입 페이지를 초기화 시킨다.
                ...
                // + 이메일 유효성
                // + 전화번호 유효성 검사
                res.send("<script>alert('중복된 아이디입니다.'); location.href='http://www.k-tech.cloudx/register.html';</script> ");
        }
    })
});
        ...
        if(err)
            ...
            connection.query("INSERT INTO board values(NULL,"+member_id+",'"+req.body.title+"','"+req.body.content+"',NOW())",function(err,result,fields){
                ...
                if(err){
                    ...
                    res.redirect('http://www.k-tech.cloud/board.html.js')
                };
            });
        }
    });
